...
from init import (
    carregar_config, ATIVOS_DIR, BOOK_DIR, GREGAS_DIR,
)
from tape import tape_carregar_ativo, tape_inicializar_ativo
...
def executar_gate(ticker: str) -> str:
    ...
    IR_GATE_E1     = 0.10
    IR_GATE_E4     = 0.20
    DD_GATE_E7     = 3.0   # drawdown máximo = 3x esperado
    ...
    if not e0_estrategia:
        ...
        raise ValueError(f"GATE bloqueado em {TICKER}: estrategias não configuradas no master JSON")
    ...
    if not e0_passou:
        ...
        raise ValueError(f"GATE bloqueado em {TICKER}: cobertura mínima insuficiente (E0)")
    ...
    print(f"  IR médio 2024: {ir_2024:+.3f}")
    print(f"  IR médio 2025: {ir_2025:+.3f}")
    print(f"  Diferença:     {abs(ir_2024-ir_2025):+.3f}")
