#!/usr/bin/env python3
"""
Simple verification script to test that the race condition fix in tape.py works correctly.
        ...
        with patch('delta_chaos.tape.ATIVOS_DIR', temp_dir):
            ...
            try:
                ...
                if backup_files:
                    ...
                    print(f"WARNING: No backup file found")
                return True
...
def test_tape_ativo_carregar_missing_file_creates_defaults():
    """Test that tape_ativo_carregar creates defaults for genuinely missing files (correct behavior)"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        with patch('delta_chaos.tape.ATIVOS_DIR', temp_dir):
            ...
            # Should create file with defaults and return them (this is CORRECT behavior for missing files)
            try:
                result = tape_ativo_carregar(ticker)
                # Verify it's a dict with expected structure
                if isinstance(result, dict) and "ticker" in result and result["ticker"] == ticker:
                    print(f"OK: Created defaults for missing file: {result['ticker']}")
                    # Verify file was actually created
                    if os.path.exists(json_path):
                        print(f"OK: File was created at {json_path}")
                    else:
...
if __name__ == "__main__":
    ...
    tests = [
        ...
        test_tape_ativo_carregar_missing_file_creates_defaults
    ]
    ...
    if passed == total:
        print("SUCCESS: ALL TESTS PASSED - TAPE FIX APPEARS TO BE WORKING!")
    else:
